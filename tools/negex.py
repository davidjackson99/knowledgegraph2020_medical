    ...
    def __init__(self, sentence='', phrases=None, rules=None,
        ...
        for i in range(len(sentenceTokens)):
            ...
            if prenFlag == 1 and overlapFlag == 0:
                sentenceTokens[i] = sentenceTokens[i].replace('[PHRASE]', '')
...
def getNegationScope(list):

    sentence = ' '.join(list)
    tagger = negTagger(sentence, list, irules, negP=False)
    scope = negTagger.getScopes(tagger)
    return scope

def removeNegationScope(list):

    sentence = ' '.join(list)
    tagger = negTagger(sentence, list, irules, negP=False)
    scope = negTagger.getScopes(tagger)
    result = sentence.replace(' '.join(scope), ' ')
    return result